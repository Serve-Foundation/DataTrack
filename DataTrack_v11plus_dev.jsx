...
// ═══ AGENCY DETAIL ═══
function AgencyDetail({ agencyId, onBack, onOpenForm, onDelete }) {
      ...
      {tab === "contacts" && (
        <div>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <button style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", backgroundColor: "#F8F6F3", color: "#525252", border: "1px solid #E8E4DF", borderRadius: 6, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: F }}>
          ...
          {agContacts.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 10 }}>
              {agContacts.map(c => (
                <div key={c.id} style={{ padding: "14px 16px", backgroundColor: "#fff", borderRadius: 8, border: "1px solid #E8E4DF" }}>
...
function DatasetList({ onNav, onAgency, initFilter }) {
  const [search, setSearch] = useState(initFilter?.search || "");
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", backgroundColor: "#0F766E", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: F }}>
...
function NotesPanel({ entityType, entityId }) {
  ...
  const entityNotes = useMemo(() => {
    const real = NOTES.filter(n => n.entity_type === entityType && n.entity_id === entityId);
    return [...localNotes.filter(n => n.entity_id === entityId), ...real];
  }, [entityType, entityId, localNotes]);
...
function RequestList({ onNav, onAgency, initFilter, onEditRecord }) {
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [search, statusF, typeF, assignedF, agencyMap, sortBy]);
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {paged.map(req => {
          ...
          const relatedComms = COMMUNICATIONS.filter(c => c.agency_id === req.agency_id && c.dataset_id === req.dataset_id);
...
function RecordForm({ type, record, onClose }) {
  ...
  const saveSupabaseBackedRecord = async (table, array, idPrefix, localRecord, dbRecord) => {
    ...
    if (!savedRecord.id) savedRecord = { ...savedRecord, id: `${idPrefix}_new_${Date.now()}` };
    const payload = { ...dbRecord, id: savedRecord.id };
    if (sb) {
      const result = isEdit
  ...
  const save = async () => {
    ...
    try {
      ...
      if (idx > -1) {
        ...
        const changed = [];
        for (const k of ["title","status","assigned_to","request_type","sent_to_contact","notes"]) { if (orig[k] !== updated[k]) changed.push(k); }
        if (changed.length > 0) {
          if (!updated.edit_log) updated.edit_log = orig.edit_log || [];
          updated.edit_log.push({ by: "Sarah Chen", date: new Date().toISOString(), fields: changed });
        }
        Object.assign(orig, updated);
      }
      else {
        updated.id = "r_new_" + Date.now();
        updated.created_at = updated.created_at || new Date().toISOString();
        updated.status = updated.status || "draft";
        updated.request_type = updated.request_type || "cpra";
...
function TaskList({ onNav, onAgency, initFilter, onViewContact }) {
  ...
  const snoozeTask = (task, days, e) => { e.stopPropagation(); const nd = new Date(task.due_date || new Date()); nd.setDate(nd.getDate() + days); const idx = TASKS.findIndex(t => t.id === task.id); if (idx > -1) { TASKS[idx].due_date = nd.toISOString().split("T")[0]; TASKS[idx].status = "open"; } setTick(t => t + 1); };
  const completeTask = (task, e) => { e.stopPropagation(); const idx = TASKS.findIndex(t => t.id === task.id); if (idx > -1) { TASKS[idx].status = "completed"; TASKS[idx].completed_at = new Date().toISOString(); } setTick(t => t + 1); };
  ...
  const save = () => {
    ...
    if (idx > -1) {
      if (TASKS[idx].assigned_to !== d.assigned_to) {
        if (!d.assignment_history) d.assignment_history = TASKS[idx].assignment_history || [];
        d.assignment_history.push({ from: TASKS[idx].assigned_to, to: d.assigned_to, date: new Date().toISOString(), by: "Sarah Chen" });
      }
      if (!d.note_history) d.note_history = TASKS[idx].note_history || [];
      Object.assign(TASKS[idx], d);
    }
    else { d.id = "task_" + Date.now(); d.assignment_history = []; d.note_history = d.note_history || []; TASKS.push(d); }
    setSaved(true);
    setTimeout(onClose, 1000);
  };
    ...
    <div style={{ backgroundColor: "#fff", borderRadius: 8, border: "1px solid #E8E4DF", overflow: "hidden" }}>
      ...
      {showNew && !submitted && (
        <div style={{ padding: "16px", backgroundColor: "#FAF9F7", borderBottom: "1px solid #E8E4DF" }}>
          ...
          <button onClick={() => { setSubmitted(true); setTimeout(() => { setShowNew(false); setSubmitted(false); setSelectedPresets([]); setCustomNote(""); }, 1200); }}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: authListener } = sb.auth.onAuthStateChange((_event, nextSession) => {
